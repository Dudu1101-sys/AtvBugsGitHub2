...
def censurar_palavra(texto, palavra_proibida):
    ...
    return texto.replace(palavra_proibida, asteriscos)  # BUG: argumentos invertidos
...
def gerar_boletim(nome, notas_bimestres):
    ...
    if media >= 6:
        ...
        nota_necessaria = (6*2) - media  # BUG: deveria ser (6*2) - media ou (12 - media) / 1
...
def relatorio_final(turma):
    ...
    percentual = len(aprovados) / len(reprovados) * total  # BUG: deveria dividir por total
